...
class Run:
    ...
    def simpleRunNN(self, epoches, batch, pace, train_ratio = 0.75):
        ...
        #data.append([[ele for ele in sample] for sample in self.data[1]])
    ...
    def train(self, data, data_label, test_data, test_data_label, clas_list, epoches, batches, pace, nn):
        ...
        plt.scatter(range(0,len(accuracyplot)),accuracyplot)
        plt.ylabel('accuracy')
        plt.title('accuracy increasing by interations')
        plt.xlabel('epoches')
        plt.show(block=True)

if __name__ == "__main__":
    #file_name = "./data/iris.data.txt"
    # above file is used for neural network
    file_name='./data/iris.data.discrete.txt'
    # above file is used for decision tree
    #file_name='./data/car.csv'
    # above file is shared by decision tree and neural network
    head = list(range(4))
    epoches = 6000
    batch = 4
    pace = 0.1
    ...
    r.simpleRunDT(train_ratio,function)
